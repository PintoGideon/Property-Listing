...
class GoogleMap extends React.Component {
	...
	showPopup(index) {
		const { markers } = this.state;
		markers[index] && markers[index].infoPopup.open(this.map, markers[index]);
	}

	hidePopup() {
		const { markers } = this.state;
		markers.forEach(marker => {
			marker.infoPopup.close();
		});
	}
	...
	createMarkers(properties) {
		...
		properties.map(property => {
			...
			this.marker.infoPopup = infoPopup;
			...
			this.showPopup(activePropertyIndex);
		});
	}
